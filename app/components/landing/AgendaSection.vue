...
<template>
  <UPageSection id="agenda" class="py-24 bg-primary-950 text-white rounded-t-[3rem] -mt-12 relative z-10">
    ...
    <!-- Event Cards Grid (Horizontal like Edukasi) -->
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
      <article
        v-for="(event, index) in events"
        :key="index"
        class="bg-primary-900/40 border border-primary-800 rounded-3xl p-2 hover:border-primary-500/50 hover:bg-primary-900/60 transition-all"
      >
        <!-- Thumbnail/Date Badge (replacing image) -->
        ...
        <!-- Event Content -->
        <div class="p-6">
          <h3 class="text-xl font-semibold mb-3 text-white line-clamp-2">{{ event.title }}</h3>
          <p class="text-primary-200/60 mb-3 text-sm">{{ event.description }}</p>
          
          <!-- Meta Info -->
          <div class="flex items-center gap-2 text-xs text-primary-200/50 mb-4">
            <UIcon v-if="event.time" name="i-lucide-clock" class="w-3.5 h-3.5" />
            <span v-if="event.time">{{ event.time }}</span>
            <UIcon v-if="event.location" name="i-lucide-map-pin" class="w-3.5 h-3.5" />
